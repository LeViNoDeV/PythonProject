...
class Button:
    ...
    def update(self, state):
        # לצייר כפתור
        #
        #  עם הרקע ירוק אז אפשר ללחוץ עליו עם לא אי אפשר
        #
        txt = ""
        filled = True
        if state == STARTUP:
...
class Chip:
    ...
    def update(self, state):
        # לצייר ציפ/להוסיף ציפ
        #
        # עם הרקע ירוק אז אפשר ללחוץ על הציפ
        # עם הצבע אדום סימן שהוא כבר נבחר
        #
        self.t.clear()
...
class Table:
    ...
    def update(self, state, ro_idx):
        ...
        if state == STARTUP:
            ...
            # לחשב הימורים ולרשום עם זכיתה בהימורים
            for i in range(44):
                is_win_pos = False
...
class Game:
    ...
    def onclick(self, x, y):
        ...
        if self.command.clickable(x, y):
            if self.state == DONE:
                # להתחיל סיבוב חדש ולמחוק את ההימורים הקודמים
                self.state = STARTUP
                self.update(self.ball.get_last_ro_index())
                ...
                if self.table.get_bet_amount() > 0:
                    ...
                    self.state = RUNNING
                    self.update(self.ball.get_last_ro_index())
                    # לסיים את הזמן אז הכדורים יפסיקו להסתובב
    ...
    def run(self):
        # לראות איך מתקדם הצייר, set "demo=True"
